...
def ImageCallBack(payload):
     if(payload == None):
          return
     chunklist = SCI_CAM_CHUNK_LIST()
     payloadAttributex = SCI_CAM_PAYLOAD_ATTRIBUTE_EX()
     nRet = SciCam_Payload_GetAttributeEx(payload,payloadAttributex)
     if nRet != SCI_CAMERA_OK:
          print("Get PayloadAttribute fail! nRet [%d]" % (nRet))
          return
     else:
          print("timestamp = [%d]: counter = [%d], framecounter = [%d],exposure = [%f],gain = [%f]\n" %(payloadAttributex.timeStamp, payloadAttributex.counter, payloadAttributex.framecounter, payloadAttributex.exposure, payloadAttributex.gain))
