...
def UpcomingThread():
     time.sleep(3)
     print("Trigger Software Once for SCI_CAMERAGrabStrategy_UpcomingImage")
     m_currentCam.SciCam_SetCommandValueEx(SciCamDeviceXmlType.SciCam_DeviceXml_Camera,"TriggerSoftware")
...
if __name__ == "__main__":
     ...
     while True:
          ...
          nRet = m_currentCam.SciCam_SetEnumValueByStringEx(SciCamDeviceXmlType.SciCam_DeviceXml_Camera,"TriggerMode" , "On")
          if nRet != SCI_CAMERA_OK:
               print("Set Trigger Mode fail! nRet [%d]" % (nRet))
               break

          #ch:设置触发源 | en:Set Trigger Source
          nRet = m_currentCam.SciCam_SetEnumValueByStringEx(SciCamDeviceXmlType.SciCam_DeviceXml_Camera,"TriggerSource" , "Software")
          if nRet != SCI_CAMERA_OK:
               print("Set Trigger Source fail! nRet [%d]" % (nRet))
               break

          print("**********************************")
          print("* 0.SciCam_GrabStrategy_OneByOne;  *")
          print("* 1.SciCam_GrabStrategy_Latest;    *")
          print("* 2.SciCam_GrabStrategy_Upcoming;  *")
          print("************************************")
          print("Please Input Grab Strategy:")
          nGrabStrategy = int(input())

          if nGrabStrategy == SciCamGrabStrategy.SciCam_GrabStrategy_OneByOne:
               print("Grab using the SciCam_GrabStrategy_OneByOne default strategy")
               #ch:设置采集策略 | en:Set Grab Strategy
               nRet = m_currentCam.SciCam_SetGrabStrategy(nGrabStrategy)
               if nRet != SCI_CAMERA_OK:
                    print("Set Grab Strategy fail! nRet [%d]" % (nRet))
                    break
          elif nGrabStrategy == SciCamGrabStrategy.SciCam_GrabStrategy_Latest:
               print("Grab using the SciCam_GrabStrategy_Latest default strategy")
               #ch:设置采集策略 | en:Set Grab Strategy
               nRet = m_currentCam.SciCam_SetGrabStrategy(nGrabStrategy)
               if nRet != SCI_CAMERA_OK:
                    print("Set Grab Strategy fail! nRet [%d]" % (nRet))
                    break
          elif nGrabStrategy == SciCamGrabStrategy.SciCam_GrabStrategy_Upcoming:
               print("Grab using the SciCam_GrabStrategy_Upcoming default strategy")
               #ch:设置采集策略 | en:Set Grab Strategy
               nRet = m_currentCam.SciCam_SetGrabStrategy(nGrabStrategy)
               if nRet != SCI_CAMERA_OK:
                    print("Set Grab Strategy fail! nRet [%d]" % (nRet))
                    break
               thread = threading.Thread(target=UpcomingThread)
               thread.start()
          else:
               print("Input error!Use default strategy:SCI_CAMERAGrabStrategy_OneByOne")
          ...
          for i in range(5):
               ...
               nRet = m_currentCam.SciCam_SetCommandValueEx(SciCamDeviceXmlType.SciCam_DeviceXml_Camera,"TriggerSoftware")
               if nRet != SCI_CAMERA_OK:
                    print("Send Trigger Software command fail! nRet [%d]" % (nRet))
                    break
               time.sleep(0.5)
          ...
          while True:
               ...
               if nRet == SCI_CAMERA_OK:
                    ...
                    print("Get PayAttribute fail! nRet [%d]" % (nRet))
